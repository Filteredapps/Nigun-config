...
BLOCKED_SONGS_FILE = ROOT / "blocked_songs.txt"
...
def main() -> None:
    ...
    feed_items = [
        ...
        if item.get("type") not in {"SINGLE", "SONG"} or item_video_id(item) not in blocked_song_ids
    ]
    popular_artist_items = dedupe_popular_artists(popular_artist_items)
    popular_song_items = [
        item for item in dedupe_popular_songs(popular_song_items)
        if item_video_id(item) not in blocked_song_ids
    ]
    ...
    feed = {
        ...
        "allowedArtistsCount": len(artists),
        "blockedSongsCount": len(blocked_song_ids),
